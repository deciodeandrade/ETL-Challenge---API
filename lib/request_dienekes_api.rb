...
module RequestDienekesApi
    ...
    def self.call
        File.open("#{self.files_path}/map_to_continue.txt", 'a')
        if IO.readlines("#{self.files_path}/map_to_continue.txt").blank?
            self.first_way
        else
        ...
        !eval(IO.readlines("#{self.files_path}/map_to_continue.txt")[0])[:is_first_time] && IO.readlines("#{self.files_path}/pages_failed.txt").blank?
    end

    def self.clear_files
        File.open("#{self.files_path}/map_to_continue.txt", 'w')
        File.open("#{self.files_path}/numbers.txt", 'w')
        File.open("#{self.files_path}/pages_failed.txt", 'w')
        File.open("#{self.files_path}/pages_failed_copy.txt", 'w')
    end
    ...
    def self.first_way
        self.clear_files
        self.with_all_pages
        self.with_pages_failed if IO.readlines("#{self.files_path}/pages_failed.txt").present? && !eval(IO.readlines("#{self.files_path}/map_to_continue.txt")[0])[:is_first_time]
    end

    def self.other_way
        last_page_success = eval(IO.readlines("#{self.files_path}/map_to_continue.txt")[0])[:last_page_success]
        if eval(IO.readlines("#{self.files_path}/map_to_continue.txt")[0])[:is_first_time]
    ...
    def self.with_pages_failed
        count = 1;
        loop do    
            pages_failed = IO.readlines("#{self.files_path}/pages_failed.txt")
            pages_failed.map!{|page| page.to_i}
            break if pages_failed == []

            File.open("#{self.files_path}/pages_failed_copy.txt", 'w') do |line|
    ...
    def self.register_numbers(page, new_array, count = 1, is_first_time = true)
        ...
        File.open("#{self.files_path}/numbers.txt", 'a') do |line|
            line.puts(new_array)
        end
    ...
    def self.register_pages_failed(page, count = 1, is_first_time = true)
        ...
        File.open("#{self.files_path}/pages_failed.txt", 'a') do |line|
            line.puts(page)
        end
        self.write_map_to_continue(is_first_time, page)
        puts "registrado"
    ...
    def self.write_map_to_continue(is_first_time = true, last_page_success = 0)
        File.open("#{self.files_path}/map_to_continue.txt", 'w') do |line|
    ...
    def self.continue_after_error_in_pages_failed(last_page_success)
        pages_failed_copy = IO.readlines("#{self.files_path}/pages_failed_copy.txt")
        ...
        File.open("#{self.files_path}/pages_failed.txt", 'w') do |line|
            line.puts(pages_failed)
        end
    end
end